...
//
// R bitmask (seg.R or seg.Ø): bit i set means column i is inherited from the
// previous row and is NOT present in the C array.  We build an activeSchema
// that excludes repeated columns and pass it to parseC.
...
function parsePBIResponse(body) {
  ...
  for (const seg of dm0) {
    ...
    // R (repeat) bitmask: bit i set → column i is inherited, not in C array.
    // Ø (U+00D8) is used for the last row in a group; treat identically.
    const repeatMask = seg.R || seg['Ø'] || 0;
    const activeSchema = schema.filter((_, i) => !(repeatMask & (1 << i)));

    // If the date column (firstT7Idx) is excluded by R, all T:7 cols in the
    // active schema are time strings — pass -1 so none are treated as timestamps.
    const activeFirstT7Idx = (repeatMask & (1 << firstT7Idx))
      ? -1
      : activeSchema.findIndex(c => c.T === 7 && !c.DN);

    const delta = parseC(seg.C, activeSchema, dicts, activeFirstT7Idx);

    const prevCourtName = currentRow.G0;
    Object.assign(currentRow, delta);

    // Emit when court changes (times may be inherited via R) or times are new.
    const courtChanged = 'G0' in delta && currentRow.G0 !== prevCourtName;
    const timesChanged = 'G3' in delta || 'G4' in delta;

    if ((courtChanged || timesChanged) && currentRow.G0?.startsWith(ALKI_PREFIX) && currentRow.G3 && currentRow.G4) {
      reservations.push({
